...
export default function DeploymentFrequencyChart({ deployments, startDate, endDate, environment }: any) {
  const options = {
    responsive: true,
    plugins: {
  ...
  const filteredDatasets = useMemo(() => {
    ...
    Object.entries(deployments).forEach(([key, value]: any) => {
      ...
      allDatasets.push({ label: key, backgroundColor: '#3c82f6', data: labels.map((l: string) => deploymentsDict[l] || 0) });
    });
